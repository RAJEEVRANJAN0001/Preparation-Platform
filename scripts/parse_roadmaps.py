...
existing_ids = ['frontend', 'backend', 'dsa', 'systemDesign', 'fullstack']
...
def parse_roadmaps(text):
    ...
    for line in lines:
        ...
        if match:
            ...
            # https://roadmap.sh/backend?r=backend-beginner -> backend-beginner
            ...
            if roadmap_id in existing_ids:
                 # Skip if we want to preserve existing handwritten ones, OR we can merge them.
                 # For now let's just create a new structure for all of them.
                 pass

            # Clean up ID for JS variable names if needed
            ...
            roadmaps[roadmap_id] = {
                "id": roadmap_id,
                "title": title,
                "description": f"Official {title} Roadmap",
                "icon": icon,
